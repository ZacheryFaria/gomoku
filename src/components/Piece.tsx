...
const Piece: React.FC<Props> = (props) => {
	...
	return (
		<circle onClick={() => setOpacity("100%")} id={`${props.x}-${props.y}`} className={placed ? "" : "Piece"} cx={props.x * cellSize} cy={props.y * cellSize} r={cellSize / 2.2} fill={color} opacity={opacity}/>
	)
}
